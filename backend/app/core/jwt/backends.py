...
class TokenBackend:
    """PyJWT를 감싸서, 토큰을 실제로 암호화 서명(encode)하고 검증(decode)하는 낮은 레벨의 로직."""
    ...
    def get_leeway(self) -> timedelta:
        ...
        if isinstance(self.leeway, timedelta):
            return self.leeway
        raise TokenBackendError(f"Unrecognized type '{type(self.leeway)}', 'leeway' must be int, float or timedelta.")
    ...
    def encode(self, payload: dict[str, Any]) -> str:
        ...
        token = jwt.encode(jwt_payload, self.prepared_signing_key, algorithm=self.algorithm)
